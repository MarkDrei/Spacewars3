...
**Part of:** Spacewars Ironstrike Architecture Documentation
...
Unlike the four persistent caches, `UserBonusCache` is a _derived cache_ — it stores computed values that can always be reconstructed from existing source data (UserCache + InventoryService). There is no novel state to persist; on server restart, bonuses are rebuilt lazily on first access.
