// ---
// PostgreSQL Database migration system
// ---
...
export const migrations: Migration[] = [
  ...
  },
  {
    ...
    up: [
      'ALTER TABLE users ADD COLUMN IF NOT EXISTS hull_current REAL NOT NULL DEFAULT 250.0',
      'ALTER TABLE users ADD COLUMN IF NOT EXISTS armor_current REAL NOT NULL DEFAULT 250.0',
      'ALTER TABLE users ADD COLUMN IF NOT EXISTS shield_current REAL NOT NULL DEFAULT 250.0',
 ...
 */
export async function applyTechMigrations(pool: Pool): Promise<void> {
  ...
  if (!needsMigration) {
    ...
    for (const statement of techMigration.up) {
      await runMigrationStatement(pool, statement);
    }
  ...
  await applyMessagesMigrations(pool);
  
  // Apply ship_hull migration
  await applyShipHullMigration(pool);
  
  // Apply defense current values migration
  await applyDefenseCurrentValuesMigration(pool);
  
  // Apply battle state migration
  await applyBattleStateMigration(pool);
  
  // Apply battle end stats migration
  await applyBattleEndStatsMigration(pool);
}
 ...
 */
export async function applyMessagesMigrations(pool: Pool): Promise<void> {
  console.log('🔄 Checking for messages table migration...');
  
  try {
    const exists = await tableExists(pool, 'messages');
    if (exists) {
 ...
 */
export async function applyShipHullMigration(pool: Pool): Promise<void> {
  console.log('🔄 Checking for ship_hull column migration...');
  
  try {
    const exists = await columnExists(pool, 'users', 'ship_hull');
    if (exists) {
 ...
 */
export async function applyDefenseCurrentValuesMigration(pool: Pool): Promise<void> {
  console.log('🔄 Checking for defense current values migration...');
  
  try {
    const exists = await columnExists(pool, 'users', 'hull_current');
    if (exists) {
 ...
 */
export async function applyBattleStateMigration(pool: Pool): Promise<void> {
  console.log('🔄 Checking for battle state migration...');
  
  try {
    const exists = await columnExists(pool, 'users', 'in_battle');
    if (exists) {
 ...
 */
export async function applyBattleEndStatsMigration(pool: Pool): Promise<void> {
  console.log('🔄 Checking for battle end stats migration...');
  
  try {
    const exists = await columnExists(pool, 'battles', 'attacker_end_stats');
    if (exists) {
