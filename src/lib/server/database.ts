...
function seedTestDatabase(db: sqlite3.Database): void {
  ...
  try {
    ...
    // This hash is precomputed for password 'a' with 10 bcrypt rounds
    const hashedPassword = '$2b$10$0q/od18qjo/fyCB8b.Dn2OZdKs1pKAOPwly98WEZzbsT.yavE6BY.';
    const techTreeJson = JSON.stringify(user.tech_tree);
    
    db.run(`
      INSERT INTO users (username, password_hash, iron, last_updated, tech_tree, ship_id, hull_current, armor_current, shield_current, defense_last_regen, ship_image_index)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [user.username, hashedPassword, user.iron, Math.floor(now / 1000), techTreeJson, shipId, 250.0, 250.0, 250.0, Math.floor(now / 1000), 1]);
