...
export const DEFAULT_USERS: SeedUser[] = [
  {
    ...
    ship: {
      x: 2500, // Center of 5000×5000 world
      y: 2500,
      speed: 0,
      angle: 0,
      picture_id: 1
...
export const DEFAULT_SPACE_OBJECTS: SeedSpaceObject[] = [
  // Asteroids - scattered around the 5000×5000 world
  ...
  try {
    ...
    for (let i = 0; i < DEFAULT_USERS.length; i++) {
      ...
      if (process.env.NODE_ENV === 'test') {
        ...
        console.log(`✅ Seeded ${DEFAULT_USERS.length + 8} users (including 8 test users) and ${DEFAULT_SPACE_OBJECTS.length + DEFAULT_USERS.length + 8} space objects for 5000×5000 world`);
      } else {
        console.log(`✅ Seeded ${DEFAULT_USERS.length} users and ${DEFAULT_SPACE_OBJECTS.length + DEFAULT_USERS.length} space objects for 5000×5000 world`);
      }
  } catch (error) {
