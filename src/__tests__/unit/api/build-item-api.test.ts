...
describe('Build Item API - input validation (unit)', () => {
  ...
  test('buildItem_countNegative_returns400', async () => {
    const sessionCookie = await createMockSessionCookie();
    const request = createRequest('http://localhost:3000/api/build-item', 'POST', {
      itemKey: 'pulse_laser',
      itemType: 'weapon',
  ...
  test('buildItem_largeCount_passesCountValidation', async () => {
    // Large counts are now allowed — iron is charged per build start, not upfront
    const sessionCookie = await createMockSessionCookie();
    const request = createRequest('http://localhost:3000/api/build-item', 'POST', {
      itemKey: 'nonexistent_weapon',
      itemType: 'weapon',
    ...
    expect(data.error).not.toContain('Count must be a positive integer');
    expect(data.error).toContain('Unknown weapon');
  });
